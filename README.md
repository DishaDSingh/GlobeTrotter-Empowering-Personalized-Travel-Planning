...
The app connects to PostgreSQL by default — install it directly, no Docker required:
...
Once installed, set the `postgres` user's password so it matches the default `DATABASE_URL`
below (skip this if you'd rather use your own credentials — just update `DATABASE_URL` to match):

```bash
# macOS/Linux
psql -U postgres -c "ALTER USER postgres PASSWORD 'root';"

# Windows (from an elevated prompt, using the psql that ships with the installer)
psql -U postgres -c "ALTER USER postgres PASSWORD 'root';"
```

That's it — no manual database creation needed. The default `postgres` database already exists,
and the backend creates all its tables in it automatically on startup.
