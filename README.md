...
**Recommended stack: Vercel (frontend) + Render (backend) + Supabase (Postgres).** All three have
free tiers, all connect straight to this GitHub repo with no CLI needed, and it's exactly what the
codebase is already set up for (`render.yaml` and `frontend/vercel.json` are included).

### 1. Database — Supabase

1. Create a project at [supabase.com](https://supabase.com) (free tier).
2. In **Project Settings → Database**, copy the connection string, "URI" tab. It looks like
   `postgresql://postgres:[password]@[host]:5432/postgres`.
3. Rewrite the scheme for SQLAlchemy's psycopg2 driver:
   `postgresql+psycopg2://postgres:[password]@[host]:5432/postgres`. This is your `DATABASE_URL`.

### 2. Backend — Render

1. Create an account at [render.com](https://render.com) and connect your GitHub account.
2. **New → Blueprint**, pick this repo. Render will read `render.yaml` at the repo root and
   propose a `globetrotter-api` web service (root dir `backend`, free plan) — accept it.
3. Before the first deploy finishes, set these environment variables on the service:
   - `DATABASE_URL` → the Supabase connection string from step 1
   - `CORS_ORIGINS` → leave as `http://localhost:5173` for now; you'll update this in step 4
     once you have the Vercel URL
   - `MAPBOX_TOKEN`, `OPENAI_API_KEY` → optional, leave blank to use the built-in fallbacks
   - `JWT_SECRET` is auto-generated by the blueprint — no action needed
4. Once deployed, note the service URL (`https://globetrotter-api-xxxx.onrender.com`). Confirm it
   with `curl https://<that-url>/health`.
5. Seed the production database once, from your machine (this points your *local* seed script at
   the *remote* Supabase DB via an env var override, then leave `backend/.env` untouched for local dev):
   ```bash
   cd backend
...
3. Add environment variable `VITE_API_URL` → your Render backend URL from step 2.4.
4. Deploy. Note the resulting URL (`https://your-project.vercel.app`).

### 4. Close the loop — CORS

Go back to the Render service's environment variables and set `CORS_ORIGINS` to your Vercel URL
(e.g. `https://your-project.vercel.app`), then trigger a redeploy (Render does this automatically
on env var changes). Without this step, the browser will block API requests from the deployed
frontend with a CORS error.

### Alternatives

- **Backend elsewhere:** Railway or Fly.io work too — any host that runs a FastAPI app behind an
  ASGI server. Same env vars apply; start command is `uvicorn app.main:app --host 0.0.0.0 --port $PORT`.
- **Frontend elsewhere:** Netlify or Cloudflare Pages also build Vite apps with zero config — set
  build command `npm run build`, publish directory `dist`, and the same `VITE_API_URL` env var.
